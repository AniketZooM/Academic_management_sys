        ...
        if role:
            ...
            if form.is_valid():
                ...
                if role == 'student':
                    ...
                    # Handle unsupported role or not assigned
...
def add_mark1(request, shift):
    try:
        ...
        return render(request, 'teacher/error.html', {'error_message': 'System error! The information you requested may not exist in the system or this function has not been implemented.'})
    ...
    if request.user.role == 'teacher':
        if page == 'add_mark':
            ...
            if request.method == 'POST':
                ...
                try:
                    if form.is_valid():
                        form.instance.teacher = request.user.teacher
                        form.save()
                        messages.success(request, f"You have successfully assigned homework for class {form.cleaned_data['for_class']} in batch {form.cleaned_data['batch']}.")
            ...
            if request.method == 'POST':
                ...
                try:
                    quiz_category.save()
                    messages.success(request, "Congrats! The quiz list and category have been added")
            ...
            if request.method == 'POST':
                ...
                try:
                    if form.is_valid():
                        form.instance.teacher = request.user.teacher
                        form.save()
                        messages.success(request, f"You have successfully assigned notes for class {form.cleaned_data['for_class']} in batch {form.cleaned_data['batch']}.")
            ...
            reminder = []
            for b in batch_of_teacher:
                if b.start_class:  # Only process if start_class exists
                    og_T = b.start_class
                    try:
                        ...
                        print(f"Error processing batch {b.id}: {str(e)}")
                        continue
...
def change_password(request):
    if request.user.role == 'student':
        if request.method == 'POST':
            ...
            if form.is_valid():
                user = form.save()
                messages.success(request, 'WELL DONE! Your password has been changed successfully')
                login(request, user)
...
def edit_profile(request):
    ...
    if request.user.role == 'teacher':
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                messages.success(request, 'Congrats! Your profile has been updated successfully')
                return redirect('edit_profile')
            else:
                messages.error(request, 'Please correct the errors below.')
        else:
    ...
    if request.user.role == "student":
        ...
        if msg_id is not None:
            try:
                ...
                return render(request, 'teacher/error.html', {'error_message': 'Error! The information you requested may not exist in the system.'})
        else:
...
def delete_notice(request, n_id):
    ...
    if request.user.teacher == note.teacher:
        note.delete()
        messages.success(request, "This notice has been deleted.")
    else:
...
def delete_note(request, note_id):
    ...
    if request.user.teacher == note.teacher:
        note.delete()
        messages.success(request, "This note has been deleted successfully.")
    else:
...
def delete_hw(request, hw_id):
    ...
    if request.user.teacher == note.teacher:
        note.delete()
        messages.success(request, "This homework has been deleted.")
    else:
...
def question_and_answer_make_by_teacher(request, q_id):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, "A new quiz category has been created")
        except Exception as e:
            return render(request, 'teacher/error.html', {'error_message': e})
...
def delete_question(request, q_id):
    try:
        ...
        if category.teacher == request.user.teacher:
            question.delete()
            messages.success(request, "This question has been deleted")
        else:
